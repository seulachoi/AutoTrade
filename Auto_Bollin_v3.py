...
while True:  
    ...
    while True:
        ...
        for ticker in tickerlist:
            ...
            if price is not None and price > bb_center and open < bb_center and close > bb_center and high_center > 0.5: 
        ...
        if len(bollin_data) == 0:
            ...
            result = f"{now}, 티커: {Bollin_coin_name}, {number}, 현재종가: {price}, {i}번째 볼밴중간값: {bb_center_df_close_str}, {i}번째 볼밴하단값: {band_low_df_close_str}, {i}번째 하단돌파%: {((df_close.iloc[-1]/band_low_df_close)-1)*100}"
            print(f"{now}, 티커: {Bollin_coin_name}, {number}, 현재종가: {price}, {i}번째 볼밴중간값: {bb_center_df_close_str}, {i}번째 볼밴하단값: {band_low_df_close_str}, {i}번째 하단돌파%: {((df_close.iloc[-1]/band_low_df_close)-1)*100}")
            
            buy_amount = krw_balance-krw_balance*0.001
            upbit.buy_market_order(Bollin_coin_name, buy_amount)   
            print(upbit.buy_market_order(Bollin_coin_name, buy_amount))
